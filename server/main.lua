    ...
    if Config.UseParkingSystem then
		...
		FindPlayerVehicles(Player.PlayerData.citizenid, function(vehicles) -- free for all
			for k, v in pairs(vehicles) do
				if type(v.plate) and plate == v.plate then isFound = true end		
			end
			if isFound then
				MySQL.Async.fetchAll("SELECT * FROM player_parking WHERE citizenid = ? AND plate = ?", {Player.PlayerData.citizenid, plate
				}, function(rs)
					if type(rs) == 'table' and #rs > 0 then
				...
				FindPlayerBoats(Player.PlayerData.citizenid, function(boats) 
			...
			if isFound then
				MySQL.Async.fetchAll("SELECT * FROM player_parking WHERE citizenid = @citizenid AND plate = @plate", {
					...
					if type(rs) == 'table' and #rs > 0 and rs[1] then
						DeleteParkedVehicle(Player.PlayerData.citizenid, plate)
				...
				FindPlayerBoats(GetCitizenid(Player), function(boats) 
					for k, v in pairs(boats) do
						if type(v.plate) and vehicleData.plate == v.plate then isFound = true end
					end
					if isFound then
						MySQL.Async.fetchAll("SELECT * FROM player_parking WHERE citizenid = ? AND plate = ?", {Player.PlayerData.citizenid, plate }, function(rs)
							if type(rs) == 'table' and #rs > 0 and rs[1] then
								DeleteParkedVehicle(Player.PlayerData.citizenid, plate)
								cb({status = true, message = Lang:t("info.has_take_the_car"), data = json.decode(rs[1].data), fuel = rs[1].fuel})
								TriggerClientEvent("qb-parking:client:deleteVehicle", -1, { plate = plate })
	...
	if Config.UseMaxParkingOnServer then
		...
		if Config.UseMaxParkingPerPlayer then -- if a player has a park limiter 
			...
			if Config.UseForVipOnly then -- only allow for vip players
				local isVip = MySQL.Sync.fetchScalar('SELECT * FROM player_parking_vips WHERE citizenid = ?', {citizenid})
...
QBCore.Commands.Add(Config.Command.addvip, Lang:t("commands.addvip"), {{name='ID', help='The id of the player you want to add.'}}, true, function(source, args)
	if args[1] and tonumber(args[1]) > 0 then
		...
		if id > 0 then
			...
			local citizenname = GetUsername(Player)
			MySQL.Async.fetchAll("SELECT * FROM player_parking_vips WHERE citizenid = ?", {citizenid}, function(rs)
				if type(rs) == 'table' and #rs > 0 then
					...
					TriggerClientEvent('QBCore:Notify', source, Lang:t('system.vip_add', {username = citizenname}), "success")
				end
			end)
		end
	end
		...
		if id > 0 then
			...
			MySQL.Async.fetchAll("SELECT * FROM player_parking_vips WHERE citizenid = ?", {citizenid}, function(rs)
				if type(rs) == 'table' and #rs > 0 then
					...
					TriggerClientEvent('QBCore:Notify', source, Lang:t('system.vip_remove', {username = citizenname}), "success")
				else
