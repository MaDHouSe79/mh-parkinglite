...
function Notify(message, type, length)
    if GetResourceState("ox_lib") ~= 'missing' then
        lib.notify({title = "MH Parking", description = message, type = type})
    else
        if Config.Framework == "qb" then
            Framework.Functions.Notify({text = "MH Parking", caption = message}, type, length)
        end
    end
end
